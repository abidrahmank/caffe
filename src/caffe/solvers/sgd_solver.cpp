...
Dtype SGDSolver<Dtype>::GetLearningRate() {
  ...
  } else if (lr_policy == "exp_growth") {
    int itr = this->iter_;
    if (itr > 0 ){
      double beta = (log(this->param_.max_lr()) - log(this->param_.base_lr())) / this->param_.max_iter();
      rate = this->param_.base_lr() * exp(beta * itr);
    } else {
        rate = this->param_.base_lr();
    }
...
void SGDSolver<Dtype>::ClipGradients() {
  ...
  if (clip_gradients >= 0) {
    ...
    l2norm_diff = std::sqrt(sumsq_diff);
    if (this->iter_ % this->param_.display() == 0) {
      LOG_IF(INFO, Caffe::root_solver()) 
                << "Iteration = " << this->iter_
  	        << "; lr = " << this->rate_
                << "; mu = " << this->momentum_
  	        << "; L2 norm of gradients = " << l2norm_diff;
    }
  }
